...
![6](../assets/post_img/2019-12-18/6.png)
...
程序测试下：
首先建立一个文件夹`OpenCV_Code`，下面新建一个cpp文件，名为`test.cpp`
...
在实际配置环境的时候我还是花了不少时间，在配置时遇到了存储空间不足、操作权限不够等问题，最后在师兄的帮助下成功完成了环境配置，开始代码优化和测试。
...
不过在分析这一部分的优化可能性中，又发现了一个难点：它在每一个循环中都计算得出了一个变量`a`，而变量`a`又作为`NNDeltaTab_i`的地址使用，这样导致访问的数据不是连续的。**在上面对NEON的介绍中，我们也知道NEON更多的是对地址连续的数据进行并行操作**，这也让这部分代码的优化变得比较难。

为了解决这一问题，我们找到了一个OpenCV封装的NEON函数：`v_lut()`，
